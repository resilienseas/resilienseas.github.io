...
### Install `oatools` R package of functions once

Only have to do this once:

```{r, eval=FALSE}
# when developing the functions with oatools in folder up, use this:
devtools::install_github("resilienseas/oatools")
```

### Load `oatools` R package

Only have to do this once:

```{r, eval=FALSE}
# when developing the functions with oatools folder, use this:
#devtools::load_all(here("../oatools"))

# versus when normally using the R library:
library(oatools)
```
...
mf_lu <- read_csv(here("./data/oah_measfreq_lookup.csv"))

mf_lu_missing = oahfocus %>%
  anti_join(
    mf_lu, by=c("MeasFreq"="MeasFreq_txt"))
# TODO: add stop() or warning() if MeasFreq not found in MeasFreq_txt
#table(mf_lu_missing$MeasFreq, useNA="ifany")

oahfocus <- oahfocus %>%
  left_join(
    mf_lu, by=c("MeasFreq"="MeasFreq_txt")) %>%
  mutate(
    MeasFreq = MeasFreq_num) %>%
  select(-MeasFreq_num)
#table(oahfocus$MeasFreq, useNA="ifany")
#oahfocus %>% select(ObjectID, MeasFreq, MeasFreq_num) %>% View()
...
r_gap_inventory <- find_gaps(oahfocus, r_sst_mean, r_sst_range, r_do_mean, r_do_range)
r_gap_carb      <- find_gaps(carbcomplete, r_sst_mean, r_sst_range, r_do_mean, r_do_range)
r_gap_highfreq  <- find_gaps(highfrequency, r_sst_mean, r_sst_range, r_do_mean, r_do_range)
```
