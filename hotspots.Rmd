---
title: "Hotspot Analysis for WCODP"
...
## Step 1. Create Predicted Aragonite Saturation State Surfaced via Kriging
#### Load packages
...
##Step 2. Create hotspot thresholds mask 
...
##Step 3. Compare predicted aragonite saturation state and hotspots to MPA Location
####Load shapefiles using pathways to your computer
```{r shapefiles, message = FALSE, warning = FALSE, results = 'hide'}

poly_MPA <- readOGR(dsn=path.expand("/Users/raetaylor-burns/Downloads/all_mpas_update"), layer="all_mpas_update") #Load MPA shapefile using pathway on personal computer
...
estuary <- readOGR(dsn=path.expand("/Users/raetaylor-burns/Downloads/estuaries"), layer="estuaries") #Load estuary shapefile using pathway on personal computer

estuary <- spTransform(estuary, crs(aragonite_raster_proj)) #Assign same CRS as aragonite layer

Canada <- readOGR(dsn=path.expand("/Users/raetaylor-burns/Downloads/Canada"), layer="Canada") #Load Canada shapefile using pathway on personal computer 
...
##Step 4. Visualizing Aragonite Saturation, Hotspots and MPA Zonal Statistics
...
pal3 <- colorRampPalette(c("royalblue1", "white", "red")) 
...
tm_shape(hotspot_clipped) +
  tm_raster(breaks= c(0, 1.1, 1.8, 2.1),
...
tm_shape(poly_MPA) +
  tm_polygons("ARAGONITE_MEAN", palette=pal(3), colorNA=NULL,
...
tm_shape(poly_MPA) +
  tm_polygons("PCT_HOTSPOTCOVER", palette=pal3(7),
