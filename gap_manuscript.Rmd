...
ext_study_alin <- extent(-670000, 340000, -650000, 350000)
ext_study_juranek <- extent(-670000, 340000, 150000, 1210000)
...
mosaic_gap_aggregate <- disaggregate(mosaic_gap, fact=10)

gap_clipped <- mask(mosaic_gap, poly_coast, inverse = TRUE) #Clip continuous raster
...
mosaic_ccgap_aggregate <- disaggregate(mosaic_carbcompletegap, fact=10)
...
mosaic_hfgap_aggregate <- disaggregate(mosaic_highfreqgap, fact=10)
...
tm_shape(gap_clipped)+
  tm_raster(palette = pal(10), colorNA = NULL, title = "High Freqency Data Gaps", alpha = 0.8, auto.palette.mapping = FALSE)+
...
tm_shape(abs())+
  tm_raster(palette = pal(6), colorNA = NULL, title = "Difference from Nearest Monitoring", auto.palette.mapping = FALSE, alpha = 0.8)+
  tm_layout(basemaps = c('OpenStreetMap'))+
...
tm_shape(Canada)+
  tm_polygons(palette = pal(10), colorNA = NULL, title = "Model Combo", auto.palette.mapping = FALSE)

tm_shape(log(r_calcite_nofill))+
  tm_raster(palette = pal(10), colorNA = NULL, auto.palette.mapping = FALSE)

tm_shape(r_ph_nofill)+
  tm_raster(palette = pal(10), colorNA = NULL, auto.palette.mapping = FALSE)
