...
ext_study_oregoncoast <- extent(-500000, 340000, 500000, 900000)
...
aragmodel <- lm(aragvar$dist ~ aragvar$gamma, data = aragvar)
intercept <- coef(aragmodel)[1]
slope <- coef(aragmodel)[2]
...
ggplot()+
  geom_point(aes(aragvar$dist, aragvar$gamma))+
  stat_smooth(aes(aragvar$dist, aragvar$gamma), method = lm)+
  theme_bw()+
...
alinaragrange <- alinaragmax-alinarag
juranekaragrange <- juranekaragmax-juranekarag
...
#crop aragonite saturation state

weight = maxValue(distance)/maxValue(oceanographicdistance)

gap<-aragrange^2*(setValues(distance, sqrt((getValues(distance)^2+(getValues(weight*oceanographicdistance)^2)))))

gap_clipped <- mask(gap_aggregate, poly_coast, inverse = TRUE) #Clip continuous raster
gap_clipped <- mask(clipped, Canada, inverse = TRUE) #Clip continuous raster
...
fullgap <- tm_shape(gap)+
  tm_raster(palette = pal(10), colorNA = NULL, alpha = 0.8, auto.palette.mapping = FALSE, legend.show = FALSE,  breaks = c(0, 20, 40, 60, 80, 100))+
  tm_layout()+
  tm_shape(states)+
  tm_polygons()+
  tm_shape(Canada)+
...
gap <- tm_shape(gap)+
  tm_raster(palette = pal(10), colorNA = NULL, title = c("Effective Distance \nto Monitoring (km)"), alpha = 0.8, auto.palette.mapping = FALSE)+
  tm_layout(main.title = "Ocean Acificitation Data Gaps", main.title.size = 1, bg.color = "white", main.title.position = c("center", "top"), legend.show = TRUE, legend.position = c("center", "BOTTOM"), legend.title.size = 1, fontfamily = "serif", legend.height = 3, fontface = "bold", legend.text.size = 0.8, legend.frame = "white", legend.frame.lwd = 1, legend.outside = TRUE)+
  tm_shape(states)+
  tm_polygons()+
  tm_shape(Canada)+
  tm_polygons()+
...
carbcompletegap <- carbcompletegap/1000

ccgap <- tm_shape(carbcompletegap)+
  tm_raster(palette = pal(10), colorNA = NULL, alpha = 0.8, auto.palette.mapping = FALSE,  breaks = c(0, 20, 40, 60, 80, 100))+
  tm_layout()+
  tm_shape(states)+
  tm_polygons()+
  tm_shape(Canada)+
...
channelislandccgap <- crop(carbcompletegap, ext_study_channelislands)

channelislandgap <- tm_shape(channelislandccgap)+
  tm_raster(palette = pal(10), colorNA = NULL, alpha = 0.8, auto.palette.mapping = FALSE, legend.show = FALSE,  breaks = c(0, 20, 40, 60, 80, 100))+
  tm_layout()+
  tm_shape(states)+
  tm_polygons()+
...
pugetsoundccgap <- crop(carbcompletegap, ext_study_pugetsound)
...
oregoncoastccgap <- crop(carbcompletegap, ext_study_oregoncoast)
...
channelislandhfgap <- crop(highfreqgap, ext_study_channelislands)

channelislandhfgap <- tm_shape(channelislandhfgap)+
  tm_raster(palette = pal(10), colorNA = NULL, alpha = 0.8, auto.palette.mapping = FALSE, legend.show = FALSE,  breaks = c(0, 20, 40, 60, 80, 100))+
  tm_layout()+
  tm_shape(states)+
  tm_polygons()+
...
pugetsoundhfgap <- crop(highfreqgap, ext_study_pugetsound)

pugetsoundgap <- tm_shape(pugetsoundhfgap)+
  tm_raster(palette = pal(10), colorNA = NULL, alpha = 0.8, auto.palette.mapping = FALSE, legend.show = FALSE,  breaks = c(0, 20, 40, 60, 80, 100))+
  tm_layout()+
  tm_shape(states)+
  tm_polygons()+
...
oregoncoasthfgap <- crop(highfreqgap, ext_study_oregoncoast)
...
hfgap <- tm_shape(highfreqgap)+
  tm_raster(palette = pal(10), colorNA = NULL, title = "Data Gaps", alpha = 0.8, auto.palette.mapping = FALSE,  breaks = c(0, 20, 40, 60, 80, 100))+
