...
ggplot(ale.seasonal, aes(date.time, aragonite)) +
  ...
  scale_color_manual(values=c("red", "pink", "white", "dodgerblue4")) +
  labs(title="Santa Barbara Channel Aragonite Saturation State\n",x="\nDate (2012 - 2013)", y = "Aragonite Saturation State\n") +
  #theme_classic() +
  theme(text=element_text(family="Spectral"), plot.title = element_text(size = 24, hjust = 0.5), axis.title = element_text(size = 18), axis.text = element_text(size = 14), legend.text=element_text(size=14), legend.title = element_text(size=14), legend.key = element_rect(fill = "grey70")) +
  theme(panel.background = element_rect(fill = "grey70",
                                        colour = "grey70"), panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
...
ggplot(ale.march, aes(date.time, aragonite)) +
  geom_line(colour = "dodgerblue3", alpha = 0.8) +
  geom_hline(yintercept = 1.7, color="red", linetype = "dashed") +
  labs(title="Santa Barbara Channel Aragonite Saturation State\n",x="\nDay (2013)", y = "Aragonite Saturation State\n") +
  theme_classic() +
  theme(text=element_text(family="Spectral"), plot.title = element_text(size = 24, hjust = 0.5), axis.title = element_text(size = 18), axis.text = element_text(size = 14)) +
  scale_x_datetime(date_breaks = "1 day", labels = date_format("%b-%d")) +
  scale_y_reverse(limits = c(3.3, 1.3)) +
  annotate("text", x = as.POSIXct('2013-03-03'), y = 1.64, label = "Biologically-critical threshold", family = "Spectral", size = 6)
