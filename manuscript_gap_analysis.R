...
tm_shape(modeldifference)+
  tm_raster(palette = pal(4), colorNA = NULL, title = "Juranek - Alin Discrepancy", auto.palette.mapping = FALSE)+
...
# devtools::load_all(here("../oatools")) # for use while developing
plot_raster(r_sst_min, "SST (C) + inventory")
...
#find oceanographic distance based on loess
...
#variogram equation: distance = (0.04/7*10^5)*semivar+0.02
#inverse: semivar = (0.04/7*10^5)*semivar+0.02

oceanographicdistance <- (vij-0.02)*(7*10^5/0.04)

carbcompleteoceanographicdistance <- (carbcompletevij-0.02)*(7*10^5/0.04)

highfreqoceanographicdistance <- (highfreqvij-0.02)*(7*10^5/0.04)
